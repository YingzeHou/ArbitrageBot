...
describe("Arbitrage", function () {
  it("get_arb_path", async function () {
    ...
    const gasPrice = 0;
    const arbitrage_path = optimal_path
    const ArbitrageOperator = await ethers.getContractFactory("ArbitrageOperator");
    const arbitrageOperator = await ArbitrageOperator.deploy(overrides = {gasPrice: gasPrice});
    // await arbitrageOperator.deployed();


    // const arbitrageTx = await arbitrageOperator.operate(overrides = {gasPrice: gasPrice}, arbitrage_path);
    // const arbitrageReceipt = await arbitrageTx.wait();
  })
})
